...
from model_adopt import GPTConfig, GPT
...
assert not (early_stopping_iters and always_save_checkpoint)
...
data_dir = os.path.join('/eagle/projects/datascience_collab/agupta/nanoGPT-HPTransfer/data/', dataset) #os.path.join('data', dataset)
def get_batch(split):
...
# optimizer
optimizer = model.configure_optimizers(optimizer_name, weight_decay, learning_rate, (beta1, beta2), adam_eps, device_type, rho)
if init_from == 'resume':
